...
def debug(data, dates, segment_dir, loc_monitor_params, write_lock):
    ...
    moitor_params = {
        k: v if type(v) != dt else v.strftime("%Y-%m-%d")
        for k, v in loc_monitor_params.items()
    }
    (save_folder / "monitor_params.json").write_text(
        json.dumps(moitor_params, indent=4)
    )

    # save dates
    (save_folder / "dates.json").write_text(
        json.dumps([d.strftime("%Y-%m-%d") for d in dates], indent=4)
    )
...
def bfast_window(
    ...
    with (segment_dir / "dates.csv").open() as f:
        dates = sorted(
            [dt.strptime(l, "%Y-%m-%d") for l in f.read().splitlines() if l.rstrip()]
        )

    # update the crop and bfast params with the current tile dates
    crop_params = {
        k: next(d for d in dates if d > val) for k, val in crop_params.items()
    }
    loc_monitor_params = {
        **monitor_params,
    ...
    monitoring_dates = dates[
        dates.index(start) : dates.index(end) + 1
    ]  # carreful slicing is x in [i,j[
    ...
    monitoring_results = np.stack(
        (decimal_breaks, model.magnitudes, model.valids)
    ).astype(np.float32)
...
def run_bfast(
    ...
    crop_params = {"start": history, "end": monitoring[1]}
    ...
    monitor_params = {
        ...
        "trend": trend,
        "level": 1
        - level,  # it's an hidden parameter I hate it https://github.com/diku-dk/bfast/issues/23
        "backend": backend,
    }
    ...
    for tile in tiles:
        ...
        if log_file.is_file():
            ...
            time.sleep(0.5)  # to let people read the message
